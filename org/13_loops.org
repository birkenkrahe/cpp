...
- Sample input:
  #+name: in:dowhile
  #+begin_src bash :results output
    echo 6566668888888888888 > ../data/dowhile
    cat ../data/dowhile
  #+end_src
...
- Code:
  ...
  #+RESULTS: pgm:dowhile
  : Enter a non-negative integer: 265604664
...
- Practice this using the practice file: modify the code to count up
  from 1 to 10. nWrite pseudocode first.
...
#+begin_src C
  int i = 3;  /* expr1 */
  while ( i > 0 /* expr2 */) {
    printf("while: T minus %d and counting\n", i-- /* expr3 */ );
   }

  for (int i = 3; i > 0; i--) {
    printf("for: T minus %d and counting\n",i);
    }
#+end_src

- Notice that we can re-use ~i~ in the =for= loop.

- Practice that now in the practice file!
...
- The program [[square]] can be condensed by converting its ~while~ loop to
  a ~for~ loop:
