...
2) If the format string for =scanf= is ="%d/%d"=, which of these two input
   ...
   #+begin_quote
   ~•5•/•96~ - =scanf= reads ~5~ and ~/~ then expects an integer or a +/-, and
   when it does not get it, it aborts, and never overwrites ~j~.

   ~•5•/96~ - =scanf= reads ~5~ and ~/~ then ~96~ as expected, and finishes.
   #+end_quote

* Week 8 - C operators / Arrays
* Week 9 - Functions
* Week 10 - Pointers
